...
import { updateGenreAPI } from '../../../apis/genreApi';

const EditGenreModal = ({ open, onClose, onSuccess, genre }) => {
  ...
  // Pre-fill form when genre changes
  useEffect(() => {
    if (genre) {
      setForm({
        name: genre.name || '',
        ...
        bannerUrl: genre.bannerUrl || '',
        category: genre.category || [],
        country: genre.country || 'Việt Nam',
        status: genre.status || 'NOW',
        isActive: genre.isActive !== false,
        duration: genre.duration || '',
        releaseDate: genre.releaseDate ? genre.releaseDate.slice(0, 10) : '',
        ageRating: genre.ageRating || 'P',
        studio: genre.studio || '',
        director: genre.director || '',
        trailerUrl: genre.trailerUrl || '',
        actors: genre.actors?.length ? genre.actors.map(a => ({ name: a.name || '', photoUrl: a.photoUrl || '' })) : [],
        stills: genre.stills || []
      });
      setErrors({});
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateGenreAPI(genre._id, {
        ...form,
  ...
  const close = () => { if (!saving) { setErrors({}); onClose(); } };
    ...
    <Dialog open={open} onClose={close} maxWidth="sm" fullWidth
      ...
      <DialogContent sx={{ px: 2.5, py: 0 }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5 }}>
          ...
          <TextField select label="Trạng thái" size="small" value={form.status || 'NOW'} onChange={set('status')} sx={sx}>
            <MenuItem value="NOW">Đang chiếu</MenuItem>
            <MenuItem value="COMING">Sắp chiếu</MenuItem>
            <MenuItem value="ARCHIVE">Lưu trữ</MenuItem>
          </TextField>
          <TextField select label="Độ tuổi" size="small" value={form.ageRating || 'P'} onChange={set('ageRating')} sx={sx}>
          ...
          <TextField select label="Hiển thị" size="small" value={String(form.isActive !== false)}
      ...
      <DialogActions sx={{ px: 2.5, py: 1.5 }}>
        ...
        <Button variant="contained" size="small" onClick={handleSubmit} disabled={saving}
          ...
          {saving ? <CircularProgress size={18} sx={{ color: '#fff' }} /> : 'Cập nhật'}
        </Button>
      </DialogActions>
    </Dialog>
  );
