...
import { Box, Typography, Skeleton, Dialog, DialogContent, IconButton, useMediaQuery, useTheme } from '@mui/material';
import {
...
export default function TicketHistory() {
  ...
  const getOrderDisplayData = (order) => {
    ...
    return { movieTitle, roomName, seatCodes, startAt, orderDate, comboText, discountText, statusInfo };
  };
  ...
  if (loading) {
    return (
      <Box sx={{ bgcolor: '#fff', p: 3 }}>
        {[1, 2, 3].map((i) => (
          <Skeleton key={i} variant="rectangular" height={isMobile ? 140 : 60} sx={{ mb: 1.5, borderRadius: 1 }} />
        ))}
      </Box>
    );
  }
        ...
        return (
          ...
          >
            ...
            <Box sx={{ px: 2, py: 1.5 }}>
              ...
              {/* Row 3: Combo (optional) */}
              {d.comboText !== '—' && (
                <Box sx={{ mb: 0.8 }}>
                  <Typography sx={{ fontSize: '0.65rem', color: 'rgba(0,0,0,0.4)', fontFamily: font, textTransform: 'uppercase', fontWeight: 600, letterSpacing: '0.5px' }}>
                    Combo
                  </Typography>
                  <Typography sx={{ fontSize: '0.75rem', color: 'rgba(0,0,0,0.6)', fontFamily: font }}>
                    {d.comboText}
                  </Typography>
                </Box>
              )}

              {/* Row 4: Giảm giá (optional) */}
              {d.discountText !== '—' && (
                <Box sx={{ mb: 0.8 }}>
                  <Typography sx={{ fontSize: '0.65rem', color: 'rgba(0,0,0,0.4)', fontFamily: font, textTransform: 'uppercase', fontWeight: 600, letterSpacing: '0.5px' }}>
                    Giảm giá
                  </Typography>
                  <Typography sx={{ fontSize: '0.75rem', color: '#2e7d32', fontFamily: font, fontWeight: 500 }}>
                    {d.discountText}
                  </Typography>
                </Box>
              )}
            ...
            <Box sx={{
              ...
              <Box
                ...
                sx={{
                  ...
                  cursor: 'pointer',
                  transition: 'all 0.2s',
                  '&:hover': { bgcolor: '#d42e80' },
                  '&:active': { transform: 'scale(0.97)' }
                }}
              >
                <QrCodeIcon sx={{ fontSize: 18 }} />
                <Typography sx={{ fontSize: '0.78rem', fontWeight: 600, fontFamily: font }}>
                  Xem vé
                </Typography>
              </Box>
            </Box>
